...
`);
 ...
 assert(html.includes('/monaco/vs/loader.js') && html.includes('id="monaco-main"') && html.includes('syncMonacoMain') && html.includes('multiCursorModifier'), 'Monaco / VS Code 同源编辑内核缺失');
  ...
  assert(html.includes('function xmindZoomAt') && html.includes('function xmindPanBy') && html.includes("handleWheel:false") && html.includes("addEventListener('wheel'") && html.includes('xmindApplyViewport'), 'XMind 画布缺少指针缩放、横纵平移或独立视口控制');
  assert(html.includes('id="pane-office"') && html.includes('id="office-workspace"') && html.includes('renderOfficeWord') && html.includes('saveOfficeWord') && html.includes('officeWordToolbar') && html.includes('contentEditable=\'true\'') && html.includes('renderOfficeSheetBook') && html.includes('renderOfficeSlides') && html.includes('/office-docx/docx-preview.min.js') && html.includes('/office-xlsx/dist/xlsx.full.min.js') && html.includes('/office-pptx/pptx-preview.umd.js'), 'Office 一级模块或本地 Word 编辑、Excel 编辑、PowerPoint 渲染能力缺失');
  ...
  assert(html.includes('rootGap=48') && html.includes('titleUnits=topic') && html.includes('rootLayouts.findIndex') && html.includes('entry.preferred.y-entry.y'), 'XMind 多棵自由导图缺少按内容高度避让与稳定排布');
  ...
  assert(html.includes('id="xmind-engine"') && html.includes('/mind-elixir/MindElixir.iife.js') && html.includes('/mind-elixir/MindElixir.css') && html.includes('new ME({') && html.includes("bus.addListener('operation'") && html.includes('xmindSyncFromEditor') && html.includes('directEditWired') && html.includes("addEventListener('dblclick'"), 'XMind 编辑模式未接入 Mind Elixir 原生编辑引擎、画布直编事件或双向保存桥接');
  ...
  assert(html.includes('森林 · 圆角') && html.includes('日落 · 便签') && html.includes('紫罗兰 · 胶囊') && html.includes('彩虹 · 分支卡片'), 'XMind 主题缺少节点形状与连线风格预设');
