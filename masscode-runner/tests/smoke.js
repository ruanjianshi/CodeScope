...
`);
  ...
  assert(['node','vault','appData','temp','dependencies','officeBuiltin'].every((key) => environment.runtime && environment.runtime[key] && environment.runtime[key].available), '运行基础环境检测不完整');
  ...
  assert(Object.values(environment.env || {}).every((item) => item.required === false && typeof item.relevant === 'boolean'), '外部项目工具仍被误标成应用运行必需项');
  ...
  assert(environment.runtime.officeBuiltin.bundled && /完整封装/.test(environment.runtime.officeBuiltin.hint), '内置 Office 没有纳入安装包运行时');
  assert(environment.env.onlyoffice && environment.env.onlyoffice.external && /内置 Office 已就绪/.test(environment.env.onlyoffice.issue) && /无需安装/.test(environment.env.onlyoffice.hint), 'ONLYOFFICE Provider 或内置编辑说明不完整');
  const officeProviders = await requestJson(baseUrl, '/api/office/providers/v1');
  assert(officeProviders.ok && officeProviders.apiRevision === 1 && officeProviders.active === 'codescope-local' && officeProviders.providers.some((item) => item.id === 'codescope-local' && item.available && item.bundled) && officeProviders.updateContract.manifestVersion === 1, 'Office Provider v1 契约异常');
