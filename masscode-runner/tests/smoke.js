...
`);
 ...
 assert(html.includes('/monaco/vs/loader.js') && html.includes('id="monaco-main"') && html.includes('syncMonacoMain') && html.includes('multiCursorModifier'), 'Monaco / VS Code 同源编辑内核缺失');
  ...
  assert(html.includes('id="xmind-engine"') && html.includes('/simple-mind-map/simpleMindMap.esm.min.js') && html.includes('/simple-mind-map/simpleMindMap.esm.min.css') && html.includes('new MindMap({') && html.includes('enableFreeDrag:false') && html.includes("XMIND_MAP.on('data_change'") && html.includes('xmindSyncFromEditor') && html.includes("addEventListener('dblclick'"), 'XMind 编辑模式未接入 simple-mind-map 原生编辑、稳定结构拖拽或双向保存桥接');
