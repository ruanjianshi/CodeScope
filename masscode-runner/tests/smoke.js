...
`);
 ...
 assert(html.includes('/monaco/vs/loader.js') && html.includes('id="monaco-main"') && html.includes('syncMonacoMain') && html.includes('multiCursorModifier'), 'Monaco / VS Code 同源编辑内核缺失');
  ...
  assert(html.includes('id="xmind-engine"') && html.includes('/mind-elixir/MindElixir.iife.js') && html.includes('/mind-elixir/MindElixir.css') && html.includes('new ME({') && html.includes("bus.addListener('operation'") && html.includes('xmindSyncFromEditor'), 'XMind 编辑模式未接入 Mind Elixir 原生编辑引擎或缺少双向保存桥接');
