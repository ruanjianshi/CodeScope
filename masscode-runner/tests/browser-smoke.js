...
`);
  ...
  await page.waitForFunction(()=>document.querySelector('.brand-version')&&document.querySelector('.brand-version').textContent==='v2.1.1 · Web');
  const versionContract=await page.evaluate(()=>fetch('/api/version').then(response=>response.json()));
  if(versionContract.version!=='2.1.1'||versionContract.apiRevision<4||versionContract.releaseChannel!=='stable'||versionContract.mode!=='web'||!versionContract.capabilities?.web||versionContract.capabilities?.desktop)throw new Error('v2.1 Web 版本契约异常：'+JSON.stringify(versionContract));
  ...
  if(!(await page.locator('#env-meta').innerText()).includes('CodeScope: v2.1.1'))throw new Error('环境元信息未显示 v2.1.1');
