...
`);
  ...
  await page.waitForFunction(()=>document.querySelector('.brand-version')&&document.querySelector('.brand-version').textContent==='v2.2.9 · Web');
  const versionContract=await page.evaluate(()=>fetch('/api/version').then(response=>response.json()));
  if(versionContract.version!=='2.2.9'||versionContract.apiRevision<5||versionContract.releaseChannel!=='stable'||versionContract.mode!=='web'||!versionContract.capabilities?.web||versionContract.capabilities?.desktop)throw new Error('v2.2 Web 版本契约异常：'+JSON.stringify(versionContract));
  ...
  if(!(await page.locator('#env-meta').innerText()).includes('CodeScope: v2.2.9'))throw new Error('环境元信息未显示 v2.2.9');
 ...
 if(ideErrors.length)throw new Error('Monaco 浏览器运行错误：'+ideErrors.join('；'));
  ...
  const zoomTools=readingPage.locator('.reading-md-zoom');if(await zoomTools.locator('.value').innerText()!=='100%')throw new Error('Markdown 缩放工具未显示默认比例');await zoomTools.locator('button').last().click();await readingPage.waitForFunction(()=>getComputedStyle(document.querySelector('.reading-md-block .ProseMirror')).zoom==='1.1');await live.dispatchEvent('wheel',{ctrlKey:true,deltaY:-100});await readingPage.waitForFunction(()=>document.querySelector('.reading-md-zoom .value').textContent==='115%');await zoomTools.locator('.value').click();await readingPage.waitForFunction(()=>getComputedStyle(document.querySelector('.reading-md-block .ProseMirror')).zoom==='1');
