...
- 阅读工作区是统一资料库，可把 PDF、DOCX、XLSX/XLS/CSV、PPTX、网页地址、Markdown、LaTeX 与代码片段组织在同一项目中并排阅读。Word 使用 docx-preview 保留分页、页眉页脚与正文结构，表格使用 SheetJS 提供工作表切换，演示文稿使用 pptx-preview 展示；网页通过受控后端提取正文、清理脚本并转换相对链接，支持打开原网页。非 PDF 资料均提供 60%–180% 独立缩放和原文件下载。PDF 阅读内核采用本地打包的 Mozilla PDF.js 官方 Viewer，支持 20%–400% 自由缩放、适合宽度、适合整页、实际大小、连续滚动、单页、双页、封面双页、旋转、手型拖动、页码跳转、全文查找、目录、缩略图、专注阅读、摘录、高亮和笔记；页面与缩略图均按可视区域懒渲染，适合数百页大文档。
...
| 📚 阅读 | 与项目、Git、标签和绘图同级的统一资料管理器，文件保存在 `vault/readings/`。左侧支持多级文件夹，项目和文件夹均可拖拽改变层级，每个项目可维护说明和标签；打开项目后，PDF 原版/译本、DOCX、XLSX/XLS/CSV、PPTX、网页、Markdown、LaTeX 与常用代码统一显示为顶部资料标签。“＋片段”可导入文档、保存网页地址或新建笔记；默认单栏，拖动标签到阅读区右侧会自动生成第二栏，关闭栏位后恢复单栏。Office 文档由本地只读解析器直接阅读并可缩放、下载，不依赖 ONLYOFFICE 服务；需要编辑时仍进入独立 Office 工作区使用 ONLYOFFICE。网页由服务端提取正文并在浏览器端再次清理危险内容。Markdown 可在源码、Milkdown Crepe 区块编辑和阅读模式间切换；代码片段可直接编辑并自动保存。PDF 支持 AI 翻译、摘录、标注和笔记，原始文件保持不变 |
