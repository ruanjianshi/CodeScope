...
当前版本：**v2.2.6**。版本变更见 [CHANGELOG.md](CHANGELOG.md)。
...
- Markdown 提供源码、分栏、区块编辑和阅读模式；分栏预览严格绑定当前文件与片段，并按源码行锚点双向同步滚动。阅读模块的区块模式使用本地打包的 Milkdown Crepe（MIT），支持直接编辑、拖拽排序、斜杠命令、选区工具栏、表格、任务列表、代码高亮、公式、原生撤销及代码/PDF 引用定位；原始 `.md` 与 frontmatter 保持兼容。
