...
- 新增空白工作区快速开始页，可直接进入全局搜索、学习分屏、阅读资料和绘图；“学习工作台 / 常用网址”固定在顶栏正中，网址使用 15 类 157 项的可搜索分类菜单，不再横向堆叠；全局工具按窗口宽度自动收纳为图标，窄屏不再拥挤或溢出。
- 统一侧栏操作热区、焦点轮廓、滚动条和高对比度模式；长项目、Office、阅读与绘图列表使用浏览器延迟绘制，降低大型 Vault 首屏排版开销。
...
- 阅读工作区是统一资料库，可把 PDF、DOCX、XLSX/XLS/CSV、PPTX、网页地址、Markdown、LaTeX 与代码片段组织在同一项目中并排阅读。Word 使用 docx-preview 保留分页、页眉页脚与正文结构，并忽略 Office 缓存的重复渲染分页标记，避免内容页之间产生空白断层；表格使用 SheetJS 提供工作表切换，演示文稿使用 pptx-preview 展示。网页通过受控后端提取正文、清理脚本并转换相对链接；文档站会恢复并持续保留完整章节目录，提供目录筛选、当前章节高亮、前进后退、上一章/下一章、相邻章节预取和栏内连续跳转。网页支持 40%–240% 整页缩放、适宽、分章位置记忆，以及带来源章节与正文范围的摘录、笔记、高亮和反向定位；Office 阅读资料提供 60%–180% 缩放和原文件下载。PDF 阅读内核采用本地打包的 Mozilla PDF.js 官方 Viewer，支持 20%–400% 自由缩放、适合宽度、适合整页、实际大小、连续滚动、单页、双页、封面双页、旋转、手型拖动、页码跳转、全文查找、目录、缩略图、专注阅读、摘录、高亮和笔记；页面与缩略图均按可视区域懒渲染，适合数百页大文档。
...
| 📚 阅读 | 与项目、Git、标签和绘图同级的统一资料管理器，文件保存在 `vault/readings/`。左侧支持多级文件夹，项目和文件夹均可拖拽改变层级，每个项目可维护说明和标签；打开项目后，PDF 原版/译本、DOCX、XLSX/XLS/CSV、PPTX、网页、Markdown、LaTeX 与常用代码统一显示为顶部资料标签。“＋片段”可导入文档、保存网页地址或新建笔记；默认单栏，拖动标签到阅读区右侧会自动生成第二栏，关闭栏位后恢复单栏。Office 文档由本地只读解析器直接阅读并可缩放、下载，不依赖 ONLYOFFICE 服务；Word 阅读会消除重复分页标记造成的隔页空白。网页采用站点阅读会话：mdBook 等文档站持续保留可筛选的完整目录，章节在阅读器内按需加载并预取相邻页，支持前后章节、历史、40%–240% 整页缩放、适宽和分章位置记忆；选中文字可保存高亮摘录或 Markdown 笔记，并可从记录切回原章节和原文位置。Markdown 可在源码、Milkdown Crepe 区块编辑和阅读模式间切换；代码片段可直接编辑并自动保存。PDF 支持 AI 翻译、摘录、标注和笔记，原始文件保持不变 |
| ▦ 学习工作台 | 顶栏正中的“学习工作台”进入 Web 多窗格空间；旁边“常用网址”按服务器、开发、科研、嵌入式、文档、AI、机械、云服务、工具、资源、视频、阅读、GitHub 项目和娱乐等分类展示，支持全库搜索、数量统计及自定义网址归类。工作台预置“左视频＋右代码/笔记”、视频｜代码、视频｜笔记和包含视频/文档/代码/笔记的真实四宫格，也可动态添加内容；标签与窗格可拖动重组，结构改变后布局显示为“自由布局”。点击 B站会打开可正常登录、搜索和选片的第一方窗口；复制视频链接后可从剪贴板载入工作台官方播放器，继续记录时间点、视频节点和定向播放器帧。普通公网网页默认进入可继续点击链接的站内阅读视图，局域网动态网页可直接嵌入。代码窗格读取并保存现有片段。 |
...
顶栏正中的“学习工作台”面向“左侧看视频/查资料，右侧写代码或笔记”的场景，布局引擎使用本地打包的 Golden Layout，不依赖外部 CDN。按钮只在工作台实际打开时高亮，再次点击即可返回代码工作区。相邻的“常用网址”是可搜索分类菜单，收纳大规模网址库；可直接新建、重命名或删除自定义分类，新增、编辑和删除网址，并通过编辑面板或拖拽把网址移动到任意分类，所有修改均保存到工作区配置。公网网页默认走站内安全阅读视图，正文和目录中的链接可继续在当前窗格导航；需要登录或运行复杂前端的页面可选择“嵌入”或“原网页”，局域网地址默认直接嵌入。拖动窗格标题可换位、叠成标签或拆成上下/左右区域；拖动分隔线调整比例，使用窗格菜单可最大化当前内容。工作区标题、网址分类、书签、隐藏项、布局与 Markdown 笔记保存在 `vault/.codescope/study/`，视频帧保存在其 `assets/` 子目录，换浏览器后仍可继续。

- `Ctrl/⌘ + Shift + 1/2/3/4`：切换四种布局预设。
- `Ctrl/⌘ + Enter`：让当前窗格进入或退出全屏专注。
- 点击顶部 B站按钮会同时进入学习工作台并打开一个正常的第一方 B站窗口；登录、搜索、收藏和自由选片都在该窗口完成，账号 Cookie 不经过 CodeScope。复制选中视频的链接后，回到工作台点击“从剪贴板载入”；普通视频链接、短链接和 BV 号均可识别。
- 播放器工具栏中的“B站浏览”会复用同一个第一方窗口并打开当前视频；“粘贴载入”可随时替换工作台中的视频。浏览器若拦截弹窗，需要允许当前 CodeScope 地址打开弹窗。
- B站播放器上方可填写播放器当前显示的秒数、`mm:ss` 或 `hh:mm:ss`：“＋时间点”插入可跳转链接，“＋视频节点”写入 `[[video:视频地址?t=秒数|标题]]`，阅读模式显示从该时间开始的 16:9 播放器卡片。
- “视频帧”只裁取当前 B站播放器区域，并把帧图片链接到对应时间点；首次使用需要选择当前 CodeScope 浏览器标签页。该能力要求 `localhost` 或 HTTPS，普通 HTTP 局域网模式仍可记录时间点和视频节点。
- 部分网站通过 `X-Frame-Options`、CSP 或第三方 Cookie 策略禁止在 iframe 中运行完整站点。工作台不会代理 B站登录凭据或绕过这些策略，而以第一方 B站窗口负责账号与选片、官方外链播放器负责学习记录。
